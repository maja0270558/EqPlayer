...
class EQSelectTrackViewController: EQScrollableViewController {
    ...
    func setupControllersAndCells() {
        visiableItemCount = 1
        guard let trackController = UIStoryboard.eqProjectStoryBoard().instantiateViewController(withIdentifier: "EQTracklistViewController") as? EQTracklistViewController else {
            return
        }
        trackController.buildViewModel(eqSettingManager, withTracks: [], isProjectController: false)
...
extension EQSelectTrackViewController: EQPlaylistTableViewControllerDelegate {
    func didSelect(playlist: SPTPartialPlaylist) {
        ...
        tracklistController?.parentViewModel.tracks.removeAllObjects()
        tracklistController?.tableView.reloadData()
        SPTPlaylistSnapshot.playlist(withURI: playlist.uri, accessToken: EQSpotifyManager.shard.auth?.session.accessToken) { [weak self] error, snapshot in
            ...
            if let snap = snapshot as? SPTPlaylistSnapshot, let trackListPage = snap.firstTrackPage.items as? [SPTPlaylistTrack] {
                self?.tracklistController?.parentViewModel.tracks = NSMutableArray(array: trackListPage)
                self?.tracklistController?.tableView.reloadDataUpdateFade()
    ...
    func getNextPageTrack(currentPage: SPTListPage) {
        currentPage.requestNextPage(withAccessToken: EQSpotifyManager.shard.auth?.session.accessToken, callback: { [weak self] _, response in
            if let page = response as? SPTListPage, let trackList = page.items as? [SPTPlaylistTrack] {
                self?.tracklistController?.parentViewModel.tracks.addObjects(from: trackList)
                self?.tracklistController?.tableView.reloadDataUpdateFade()
                if page.hasNextPage {
...
extension EQSelectTrackViewController: UISearchBarDelegate, EQSearchViewControllerDelegate {
    func didDismiss() {
        tracklistController?.tableView.reloadDataUpdateFade()
        ...
        UIView.animate(withDuration: 0.25, animations: { [weak self] in
            self?.moveUpAnimation()
                ...
                if let searchViewController = UIStoryboard.eqProjectStoryBoard().instantiateViewController(withIdentifier: String(describing: EQSearchViewController.self)) as? EQSearchViewController {
                    ...
                    searchViewController.buildViewModel(self?.eqSettingManager!, withTracks: [], isProjectController: false)
