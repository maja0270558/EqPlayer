...
class EQUserViewController: UIViewController {
    @IBOutlet var userInfoTopView: UIView!
    @IBOutlet var userInfoView: EQUserInfoView!
    @IBOutlet var toolBarView: EQCustomToolBarView!
    ...
    func setupUserInfoView() {
        ...
        userInfoView.cameraButton.addTarget(self, action: #selector(changeProfilePhoto), for: .touchUpInside)
        ...
        moreOptionAlert(
            delete: { _ in
                ...
                if let eqProjectViewController = UIStoryboard.eqProjectStoryBoard().instantiateViewController(withIdentifier: String(describing: EQProjectViewController.self)) as? EQProjectViewController {
                    let projectModel = EQSettingModelManager(model: data)
                    ...
                    eqProjectViewController.buildViewModel(projectModel, withTracks: NSMutableArray(array: Array(projectModel.tempModel.tracks)),isProjectController: true)
...
extension EQUserViewController: EQProjectTableViewControllerDelegate {
    ...
    func didSelectProjectCell(at _: IndexPath, data: EQProjectModelProtocol) {
